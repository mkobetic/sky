...
func TestServerInsertEventTooLarge(t *testing.T) {
	runTestServer(func(s *Server) {
		...
		assertResponse(t, resp, 500, `{"message":"lmdb txn put error: MDB_BAD_VALSIZE: Too big key/data, key is empty, or wrong DUPFIXED size (len=613)"}`+"\n", "Too large event")
	})
}
...
func TestServerStreamEventTooLarge(t *testing.T) {
	runTestServer(func(s *Server) {
		...
		assertResponse(t, resp, 500, `{"message":"Cannot put event: lmdb txn put error: MDB_BAD_VALSIZE: Too big key/data, key is empty, or wrong DUPFIXED size (len=613)", "events_written":0}`, "Stream event too large")
	})
}
